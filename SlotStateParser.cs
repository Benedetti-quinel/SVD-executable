...
namespace WpfApp1
{
    internal class SlotStateParser
    {
        ...
        public List<TabInfo> Parse()
        {
            ...
            foreach (XmlNode state in states)
            {
                ...
                newTabInfo.Reelset = state.SelectSingleNode("reelset").InnerText;

                XmlNode stops = state.SelectSingleNode("stops");
                String[] stopString = stops.InnerText.Split(',');
                int row = 0;
                foreach (String stop in stopString)
                {
                    newTabInfo.Stops.Add(stop);
                    row++;
                }
